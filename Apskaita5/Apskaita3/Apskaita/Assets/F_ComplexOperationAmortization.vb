...
Public Class F_ComplexOperationAmortization
    Implements ISupportsPrinting, IObjectEditForm

    Private Obj As ComplexOperationAmortization
    ...
    Private Sub F_OperationAmortization_Load(ByVal sender As System.Object, _
        ...
        Try
            If _OperationID > 0 Then
                Obj = LoadObject(Of ComplexOperationAmortization)(Nothing, _
                    "GetComplexOperationAmortization", False, _OperationID, _
                    _FetchByJournalEntry)
            Else
                Obj = ComplexOperationAmortization.NewComplexOperationAmortization()
            End If
    ...
    Private Sub CalculateAmortizationButton_Click(ByVal sender As System.Object, _
        ...
        Try
            ...
            Using busy As New StatusBusy
                ...
                Dim calculations As LongTermAssetAmortizationCalculationList _
                    = Obj.GetCalculations()
                Obj.SetCalculations(calculations, warnings)
    ...
    Private Sub ItemsDataGridView_CellBeginEdit(ByVal sender As Object, _
        ...
        Dim current As OperationAmortization = Nothing
        Try
            current = DirectCast(ItemsDataGridView.Rows(e.RowIndex).DataBoundItem, OperationAmortization)
        Catch ex As Exception
        End Try
        ...
        If (ItemsDataGridView.Columns(e.ColumnIndex) Is TotalValueChangeDataGridViewColumn _
            AndAlso current.TotalValueChangeIsReadOnly) OrElse _
            (ItemsDataGridView.Columns(e.ColumnIndex) Is UnitValueChangeDataGridViewColumn _
            AndAlso current.UnitValueChangeIsReadOnly) OrElse _
            (ItemsDataGridView.Columns(e.ColumnIndex) Is AmortizationCalculatedForMonthsDataGridViewColumn _
            AndAlso current.AmortizationCalculatedForMonthsIsReadOnly) OrElse _
            (ItemsDataGridView.Columns(e.ColumnIndex) Is RevaluedPortionTotalValueChangeDataGridViewColumn _
            AndAlso current.RevaluedPortionTotalValueChangeIsReadOnly) OrElse _
            (ItemsDataGridView.Columns(e.ColumnIndex) Is RevaluedPortionUnitValueChangeDataGridViewColumn _
            AndAlso current.RevaluedPortionUnitValueChangeIsReadOnly) OrElse _
            (ItemsDataGridView.Columns(e.ColumnIndex) Is AmortizationCalculationsDataGridViewColumn _
            AndAlso current.AmortizationCalculationsIsReadOnly) Then
    ...
    Private Sub ItemsDataGridView_CellDoubleClick(ByVal sender As Object, _
        ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) _
        Handles ItemsDataGridView.CellDoubleClick

        If e.RowIndex < 0 OrElse Obj Is Nothing Then Exit Sub

        If ItemsDataGridView.Columns(e.ColumnIndex) Is TotalValueChangeDataGridViewColumn OrElse _
            ItemsDataGridView.Columns(e.ColumnIndex) Is UnitValueChangeDataGridViewColumn OrElse _
            ItemsDataGridView.Columns(e.ColumnIndex) Is AmortizationCalculatedForMonthsDataGridViewColumn OrElse _
            ItemsDataGridView.Columns(e.ColumnIndex) Is RevaluedPortionTotalValueChangeDataGridViewColumn OrElse _
            ItemsDataGridView.Columns(e.ColumnIndex) Is RevaluedPortionUnitValueChangeDataGridViewColumn OrElse _
            ItemsDataGridView.Columns(e.ColumnIndex) Is AmortizationCalculationsDataGridViewColumn Then
            Exit Sub
        End If
        ...
        If Not YesOrNo(String.Format("Gauti amortizacijos paskaičiavimą turtui ""{0}""?", _
            current.AssetName)) Then Exit Sub
        ...
        Try
            ...
            Dim calculation As LongTermAssetAmortizationCalculation _
                ...
                current.AssetID, current.ID, Obj.Date)
            Obj.SetCalculations(calculation)
    ...
    Private Sub ItemsDataGridView_UserDeletingRow(ByVal sender As Object, _
        ...
        Dim current As OperationAmortization = Nothing
        Try
            current = DirectCast(e.Row.DataBoundItem, OperationAmortization)
        Catch ex As Exception
        End Try
        If current Is Nothing Then Exit Sub
    ...
    Private Function SaveObj() As Boolean
        ...
        Using bm As New BindingsManager(ComplexOperationAmortizationBindingSource, _
            ItemsSortedBindingSource, Nothing, True, False)

            Try
                Obj = LoadObject(Of ComplexOperationAmortization)(Obj, "Save", False)
            Catch ex As Exception
                ShowError(ex)
                Return False
            End Try
    ...
    Private Sub ConfigureButtons()
        ...
        GetCalculationsButton.Enabled = (Not Obj Is Nothing AndAlso _
