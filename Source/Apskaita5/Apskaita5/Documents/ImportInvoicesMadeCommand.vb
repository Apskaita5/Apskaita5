...
Namespace Documents
    ...
    ''' <summary>
    ''' Represents a command that imports invoices made from external systems (integration end point).
    ''' </summary>
    <Serializable()> _
Public NotInheritable Class ImportInvoicesMadeCommand
...
#Region " Factory Methods "
        ...
        ''' <summary>
        ''' Imports invoices made from an external system (integration end point).
        ''' </summary>
        ...
        Public Shared Function TheCommand(ByVal cInvoices As List(Of InvoiceInfo.InvoiceInfo)) As String
            ...
            if (cInvoices Is Nothing OrElse cInvoices.Count < 1) Then _
                Throw New Exception("Nenurodyta nė viena importuotina išrašyta sąskaita faktūra.")

            Dim cmd As New ImportInvoicesMadeCommand
            cmd._Invoices = cInvoices

            cmd.BeforeServer()
            cmd = DataPortal.Execute(Of ImportInvoicesMadeCommand)(cmd)
            cmd.AfterServer()
...
#Region " Server-side Code "
        ...
        Protected Overrides Sub DataPortal_Execute()
            ...
            Dim invoicesToSave As new List(Of KeyValuePair(of InvoiceMade, Person))
            For Each invoice As InvoiceInfo.InvoiceInfo In Invoices
                invoicesToSave.Add(GetOrCreateInvoice(invoice, clientList, accountList, _
                    vatSchemaList, serviceList))
            Next

            Dim newPersons As New List(Of Person)
            Dim newPersonCodes As New List(Of String)
            For Each invoice As KeyValuePair(Of InvoiceMade,Person) In invoicesToSave
            ...
            Dim current as InvoiceMade = Nothing
                    ...
                    Try
                        ...
                        For Each invoice As KeyValuePair(Of InvoiceMade,Person) In invoicesToSave
                            ...
                            If Not invoice.Value Is Nothing Then
                                invoice.Key.SetImportedPayer(PersonInfo.GetPersonInfo( _
        ...
        Friend Shared Function GetOrCreateInvoice(invoice As InvoiceInfo.InvoiceInfo, _
            ...
            vatSchemaList As VatDeclarationSchemaInfoList, serviceList As ServiceInfoList) As KeyValuePair(of InvoiceMade, Person)
            ...
            If StringIsNullOrEmpty(invoice.Content) Then
                invoice.Content = "Išrašyta sąskaita faktūra"
            End If
            If invoice.Payer Is Nothing OrElse StringIsNullOrEmpty(invoice.Payer.Code) Then
                Throw New Exception(String.Format("Nenurodytas pirkėjo kodas.{0}------InvoiceData------{1}{2}", _
                    vbCrLf, vbCrLf, InvoiceInfo.ToXmlString(invoice)))
            End If
            If invoice.Payer Is Nothing OrElse StringIsNullOrEmpty(invoice.Payer.Name) Then
                Throw New Exception(String.Format("Nenurodytas pirkėjo pavadinimas.{0}------InvoiceData------{1}{2}", _
                    vbCrLf, vbCrLf, InvoiceInfo.ToXmlString(invoice)))
            End If
            ...
            Dim result As InvoiceMade

            Try
                result = InvoiceMade.GetOrCreateInvoiceMadeChild(invoice, False, _
                    clientList, accountList, vatSchemaList, serviceList, newClientInfo)  
            Catch ex As Exception
            ...
            If result.Payer = PersonInfo.Empty Then
                ...
                invoice.Payer.IsClient = true
                invoice.Payer.IsSupplier = false
                invoice.Payer.IsWorker = False

                newClient = Person.NewPersonChild(invoice.Payer)

                If Not newClient.IsValid Then Throw New Exception(String.Format( _
                    "Nepakanka duomenų naujo kliento įtraukimui:{0}{1}{2}------InvoiceData------{3}{4}", _
                    vbCrLf, newClient.GetAllBrokenRules(), vbCrLf, vbCrLf, InvoiceInfo.ToXmlString(invoice)))

            End If

            Return new KeyValuePair(Of InvoiceMade,Person)(result, newClient)

        End Function

        Friend Shared Sub SaveImportedInvoice(invoice As KeyValuePair(of InvoiceMade, Person))

            If Not invoice.Value Is Nothing Then
                Dim newClient = invoice.Value.SaveChild()
                invoice.Key.SetImportedPayer(PersonInfo.GetPersonInfo(newClient))
            End If
