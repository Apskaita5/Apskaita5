...
Imports System.Xml
Imports ApskaitaObjects.ActiveReports.Declarations.ISafTemplates.ISaf_1_2
Imports ApskaitaObjects.My.Resources

Namespace ActiveReports.Declarations

    ''' <summary>
    ''' Represents an implementation of an <see cref="IInvoiceRegisterISaf">IInvoiceRegisterISaf</see>
    ''' for a state tax inspectorate (VMI) i.SAF report version 1.1.
    ''' </summary>
    ...
    Public Class InvoiceRegisterISaf_1
        ...
        Public ReadOnly Property Name() As String _
            Implements IInvoiceRegisterISaf.Name
            Get
                Return "SAF-T v. 1.2"
            End Get
        End Property
        ...
        Public ReadOnly Property ValidFrom() As Date _
            Implements IInvoiceRegisterISaf.ValidFrom
            Get
        ...
        Public ReadOnly Property ValidTo() As Date _
            Implements IInvoiceRegisterISaf.ValidTo
            Get
        ...
        Public ReadOnly Property XsdFileName() As String _
            Implements IInvoiceRegisterISaf.XsdFileName
            Get
        ...
        ''' <remarks></remarks>
        Public Function GetXmlString(ByVal invoiceRegister As InvoiceInfoItemList,
            ByVal softwareVersion As String, ByVal selectedInvoicesIds As Integer(),
            ByRef warnings As String) As String _
            Implements IInvoiceRegisterISaf.GetXmlString
            ...
            Dim result As New ISafTemplates.ISaf_1_2.iSAFFile

            result.Header = New ISafTemplates.ISaf_1_2.Header()
            result.Header.FileDescription = New ISafTemplates.ISaf_1_2.FileDescription()
            If invoiceType = InvoiceInfoType.InvoiceMade Then
                result.Header.FileDescription.DataType =
                    ISafTemplates.ISaf_1_2.ISAFDataType.S
            Else
                result.Header.FileDescription.DataType =
                    ISafTemplates.ISaf_1_2.ISAFDataType.P
            End If
            result.Header.FileDescription.FileDateCreated = Now
            result.Header.FileDescription.FileVersion =
                ISafTemplates.ISaf_1_2.ISAFFileVersion.iSAF12
            result.Header.FileDescription.NumberOfParts = 2
            result.Header.FileDescription.PartNumber = invoiceType.ToString().ToUpper()
            result.Header.FileDescription.RegistrationNumber =
            ...
            result.Header.FileDescription.SelectionCriteria =
                New ISafTemplates.ISaf_1_2.SelectionCriteria()
            ...
            result.SourceDocuments = New ISafTemplates.ISaf_1_2.SourceDocuments()
            result.MasterFiles = New ISafTemplates.ISaf_1_2.MasterFiles()

            If invoiceType = InvoiceInfoType.InvoiceMade Then
                result.SourceDocuments.SalesInvoices = GetSalesInvoices(invoiceRegister, selectedInvoicesIds)
                result.MasterFiles.Customers = GetCustomers(
            ...
            Else
                ...
                result.MasterFiles.Suppliers = GetSuppliers(
            ...
            If Not XmlValidationErrorBuilder.Validate(resultString,
                IO.Path.Combine(AppPath(), Me.XsdFileName),
        ...
        Private Function GetPurchaseInvoices(ByVal invoiceRegister As InvoiceInfoItemList,
            ByVal selectedInvoicesIds As Integer()) As ISafTemplates.ISaf_1_2.PurchaseInvoice()

            Dim result As New List(Of ISafTemplates.ISaf_1_2.PurchaseInvoice)

            For Each item As InvoiceInfoItem In invoiceRegister

                If selectedInvoicesIds Is Nothing OrElse selectedInvoicesIds.Length < 1 OrElse
                    Not Array.IndexOf(selectedInvoicesIds, item.ID) < 0 Then

                    Dim invoice As New ISafTemplates.ISaf_1_2.PurchaseInvoice

                    invoice.References = New ISafTemplates.ISaf_1_2.Reference() {}
                    invoice.InvoiceDate = item.ActualDate
                    invoice.InvoiceNo = item.Number
                    If item.InvoiceType = Documents.InvoiceType.Credit Then
                        invoice.InvoiceType = ISafTemplates.ISaf_1_2.ISAFshorttext2Type.KS
                    ElseIf item.InvoiceType = Documents.InvoiceType.Debit Then
                        invoice.InvoiceType = ISafTemplates.ISaf_1_2.ISAFshorttext2Type.DS
                    Else
                        invoice.InvoiceType = ISafTemplates.ISaf_1_2.ISAFshorttext2Type.SF
                    End If
                    ...
                    invoice.SupplierInfo = New ISafTemplates.ISaf_1_2.SupplierInfo()
        ...
        Private Function GetSalesInvoices(ByVal invoiceRegister As InvoiceInfoItemList,
            ByVal selectedInvoicesIds As Integer()) As ISafTemplates.ISaf_1_2.SalesInvoice()

            Dim result As New List(Of ISafTemplates.ISaf_1_2.SalesInvoice)
            ...
            For Each item As InvoiceInfoItem In invoiceRegister
                ...
                If selectedInvoicesIds Is Nothing OrElse selectedInvoicesIds.Length < 1 OrElse
                    ...
                    Dim invoice As New ISafTemplates.ISaf_1_2.SalesInvoice
                    ...
                    invoice.CustomerInfo = New ISafTemplates.ISaf_1_2.CustomerInfo()
                    ...
                    Dim subtotals As New List(Of ISafTemplates.ISaf_1_2.SalesDocumentTotal)
                    For Each subitem As InvoiceSubtotalItem In item.SubtotalList

                        Dim subtotal As New ISafTemplates.ISaf_1_2.SalesDocumentTotal
        ...
        Private Function GetCustomers(ByVal salesInvoices As ISafTemplates.ISaf_1_2.SalesInvoice()) _
            As ISafTemplates.ISaf_1_2.Customer()

            Dim result As New List(Of ISafTemplates.ISaf_1_2.Customer)

            Dim personAlreadyAdded As Boolean

            For Each invoice As ISafTemplates.ISaf_1_2.SalesInvoice In salesInvoices

                personAlreadyAdded = False

                For Each item As ISafTemplates.ISaf_1_2.Customer In result
                ...
                If Not personAlreadyAdded Then
                    ...
                    Dim newPerson As New ISafTemplates.ISaf_1_2.Customer
        ...
        Private Function GetSuppliers(ByVal purchaseInvoices As ISafTemplates.ISaf_1_2.PurchaseInvoice()) _
            As ISafTemplates.ISaf_1_2.Supplier()

            Dim result As New List(Of ISafTemplates.ISaf_1_2.Supplier)

            Dim personAlreadyAdded As Boolean

            For Each invoice As ISafTemplates.ISaf_1_2.PurchaseInvoice In purchaseInvoices

                personAlreadyAdded = False

                For Each item As ISafTemplates.ISaf_1_2.Supplier In result
                ...
                If Not personAlreadyAdded Then
                    ...
                    Dim newPerson As New ISafTemplates.ISaf_1_2.Supplier
            ...
            For Each item As InvoiceInfoItem In invoiceRegister
                If selectedInvoicesIds Is Nothing OrElse selectedInvoicesIds.Length < 1 _
                    ...
                    If item.SubtotalList.Count < 1 Then
                        errors = AddWithNewLine(errors, String.Format(ActiveReports_Declarations_InvoiceRegisterSafT_1_VatSchemaNull,
                    ...
                    Else
                        For Each o As InvoiceSubtotalItem In item.SubtotalList
                            If StringIsNullOrEmpty(o.TaxCode) Then
                                errors = AddWithNewLine(errors, String.Format(ActiveReports_Declarations_InvoiceRegisterSafT_1_VatSchemaWithoutVatCode,
                                    item.Date.ToString("yyyy-MM-dd"), item.Number), False)
                            ElseIf Not ValidateVatCode(o.TaxCode) Then
                                errors = AddWithNewLine(errors, String.Format(ActiveReports_Declarations_InvoiceRegisterSafT_1_VatCodeInvalid,
