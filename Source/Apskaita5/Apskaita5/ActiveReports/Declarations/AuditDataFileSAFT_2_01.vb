...
Imports System.Xml
Imports ApskaitaObjects.ActiveReports.Declarations.SafTTemplates.SAFT_2_01
...
Namespace ActiveReports.Declarations
    ...
    ''' for a state tax inspectorate (VMI) SAF-T report version 2.01
    ''' </summary>
    ...
    Public NotInheritable Class AuditDataFileSAFT_2_01
        ...
        Public Function GetXmlString(ByVal softwareVersion As String, ByVal periodStart As Date,
            ...
            result.GeneralLedgerEntries.Journals = New AuditFileGeneralLedgerEntriesJournal() {New AuditFileGeneralLedgerEntriesJournal}
            result.GeneralLedgerEntries.Journals(0).Description = "Bendrasis žurnalas"
            result.GeneralLedgerEntries.Journals(0).JournalID = "BŽ"
            result.GeneralLedgerEntries.Journals(0).Type = "BŽ"
            Dim transactionCount As Integer = 0
            Dim debitSum As Double = 0.0
            Dim creditSum As Double = 0.0
            result.GeneralLedgerEntries.Journals(0).Transactions = GetGeneralLedgerTransactions(
        ...
        Private Function GetHeader(ByVal softwareVersion As String, ByVal periodStart As Date,
            ...
            result.SelectionCriteria.PeriodStart = periodStart
            result.SelectionCriteria.PeriodEnd = periodEnd
            result.SelectionCriteria.SelectionStartDate = periodStart
            result.SelectionCriteria.SelectionEndDate = periodEnd
        ...
        Private Function GetSuppliers(myData As DataTable) As AuditFileMasterFilesSupplier()
            ...
            For Each dr As DataRow In myData.Rows
                Dim prs As New AuditFileMasterFilesSupplier
                    ...
                    prs.Accounts = New AuditFileMasterFilesSupplierAccounts() {New AuditFileMasterFilesSupplierAccounts()}
                    prs.Accounts(0).AccountID = CLongSafe(dr.Item(9), 0).ToString
                    prs.SelfBillingIndicator = ""

                    If CDblSafe(dr.Item(10), 2, 0.0) > 0 Then
                        prs.Accounts(0).Item = Convert.ToDecimal(CDblSafe(dr.Item(10), 2, 0.0))
                        prs.Accounts(0).ItemElementName = ItemChoiceType3.OpeningDebitBalance
                    Else
                        prs.Accounts(0).Item = Convert.ToDecimal(-CDblSafe(dr.Item(10), 2, 0.0))
                        prs.Accounts(0).ItemElementName = ItemChoiceType3.OpeningCreditBalance
                    End If
                    If CDblSafe(dr.Item(11), 2, 0.0) > 0 Then
                        prs.Accounts(0).Item1 = Convert.ToDecimal(CDblSafe(dr.Item(11), 2, 0.0))
                        prs.Accounts(0).Item1ElementName = Item1ChoiceType2.ClosingDebitBalance
                    Else
                        prs.Accounts(0).Item1 = Convert.ToDecimal(-CDblSafe(dr.Item(11), 2, 0.0))
                        prs.Accounts(0).Item1ElementName = Item1ChoiceType2.ClosingCreditBalance
                    End If
        ...
        Private Function GetTaxTable() As AuditFileMasterFilesTaxTableEntry()
            ...
            Dim result As New List(Of AuditFileMasterFilesTaxTableEntryTaxCodeDetail)

            Dim myComm As New SQLCommand("FetchSaftTaxTable_2_0")
            Using myData As DataTable = myComm.Fetch
                For Each dr As DataRow In myData.Rows

                    Dim entry As New AuditFileMasterFilesTaxTableEntryTaxCodeDetail
        ...
        Private Function GetProducts(periodEnd As Date) As AuditFileMasterFilesProduct()
            ...
            Using myData As DataTable = myComm.Fetch
                For Each dr As DataRow In myData.Rows
                    ...
                    entry.UOMS = New AuditFileMasterFilesProductUOM() {New AuditFileMasterFilesProductUOM()}
                    entry.UOMS(0).UOMStandard = CStrSafe(dr.Item(3))
                    entry.UOMS(0).UOMToUOMBaseConversionFactor = New System.Nullable(Of Decimal)(Convert.ToDecimal(1.0))
                    ...
                    If taxId1 > 0 AndAlso taxId2 > 0 Then
                        ...
                        entry.Taxes = New AuditFileMasterFilesProductTax() {New AuditFileMasterFilesProductTax, New AuditFileMasterFilesProductTax}
                        entry.Taxes(0).TaxType = "PVM"
                        entry.Taxes(0).TaxCode = taxId1.ToString
                        entry.Taxes(1).TaxType = "PVM"
                        entry.Taxes(1).TaxCode = taxId2.ToString

                    ElseIf taxId1 > 0 Then

                        entry.Taxes = New AuditFileMasterFilesProductTax() {New AuditFileMasterFilesProductTax}
                        entry.Taxes(0).TaxType = "PVM"
                        entry.Taxes(0).TaxCode = taxId1.ToString
                    ...
                    ElseIf taxId2 > 0 Then
                        ...
                        entry.Taxes = New AuditFileMasterFilesProductTax() {New AuditFileMasterFilesProductTax}
                        entry.Taxes(0).TaxType = "PVM"
                        entry.Taxes(0).TaxCode = taxId2.ToString

                    Else

                        entry.Taxes = New AuditFileMasterFilesProductTax() {}
            ...
            Using myData As DataTable = myComm.Fetch
                ...
                Using consignmentsData As DataTable = myComm.Fetch
                    ...
                    For Each dr As DataRow In myData.Rows
                        ...
                        entry.PhysicalStockAcquisitions = acquisitions.ToArray
        ...
        Private Function GetAssets(ByVal periodStart As Date,
            ...
            Using myData As DataTable = myComm.Fetch
                ...
                For Each dr As DataRow In myData.Rows
                    ...
                    asset.Suppliers = New AuditFileMasterFilesAssetSupplier() {New AuditFileMasterFilesAssetSupplier}
                    asset.Suppliers(0).SupplierID = CIntSafe(dr.Item(3), 0).ToString
                    asset.Suppliers(0).SupplierName = CStrSafe(dr.Item(4))
                    asset.Suppliers(0).PostalAddress = New AddressStructure
                    asset.Suppliers(0).PostalAddress.AddressType = AddressStructureAddressType.BA
                    asset.Suppliers(0).PostalAddress.FullAddress = CStrSafe(dr.Item(5))
                    asset.Suppliers(0).PostalAddress.Country = CStrSafe(dr.Item(6))
                    If String.IsNullOrEmpty(asset.Suppliers(0).PostalAddress.Country.Trim) Then _
                        asset.Suppliers(0).PostalAddress.Country = StateCodeLith
                    asset.Suppliers(0).PostalAddress.City = _NotSpecifiedPlaceHolder
                    asset.Suppliers(0).PostalAddress.Number = _NotSpecifiedPlaceHolder
                    asset.Suppliers(0).PostalAddress.StreetName = _NotSpecifiedPlaceHolder
                    asset.Suppliers(0).PostalAddress.PostalCode = _NotSpecifiedPlaceHolder
                    ...
                    If appreciationDate <> Date.MaxValue AndAlso appreciation > 0 Then
                        ...
                        asset.Valuations(0).Appreciations(0).AppreciationSum = Convert.ToDecimal(appreciation)
                        asset.Valuations(0).Appreciations(0).AppreciationDate = appreciationDate
                        asset.Valuations(0).Appreciations(0).AppreciationDateSpecified = True
                        asset.Valuations(0).Appreciations(0).AppreciationSumSpecified = True
                        asset.Valuations(0).Appreciations(0).DepreciationForPeriod = Convert.ToDecimal(CDblSafe(dr.Item(30), 2, 0))
                        asset.Valuations(0).Appreciations(0).DepreciationForPeriodSpecified = True
                        asset.Valuations(0).Appreciations(0).DepreciationMethod = AuditFileMasterFilesAssetValuationAppreciationDepreciationMethod.T
                        asset.Valuations(0).Appreciations(0).DepreciationMethodSpecified = True
                    End If
        ...
        Private Function GetOwners(periodEnd As Date) As AuditFileMasterFilesOwner()
            ...
            Using myData As DataTable = myComm.Fetch
                For Each dr As DataRow In myData.Rows
                    ...
                    entry.SharesQuantity = Convert.ToDecimal(quantity)
            ...
            Using myData As DataTable = myComm.Fetch
                ...
                Using detailsData As DataTable = myComm.Fetch
                    ...
                    For Each dr As DataRow In myData.Rows
                        ...
                        transaction.Lines = lines.ToArray
                ...
                Using detailsData As DataTable = myComm.Fetch
                    ...
                    For Each dr As DataRow In myData.Rows
                        ...
                        If payment.PaymentMethod = AuditFileSourceDocumentsPaymentsPaymentPaymentMethod.BP Then
                            ...
                            payment.Lines = New AuditFileSourceDocumentsPaymentsPaymentLine() _
                                {New AuditFileSourceDocumentsPaymentsPaymentLine}
                            payment.Lines(0).PaymentLineAmount = New AmountStructure
                            payment.Lines(0).PaymentLineAmount.Amount = payment.GrossTotal
                            payment.Lines(0).PaymentLineAmount.CurrencyAmount = Convert.ToDecimal(
                                CDblSafe(dr.Item(5), 2, 0.0))
                            payment.Lines(0).PaymentLineAmount.CurrencyCode = CStrSafe(dr.Item(6))
                            payment.Lines(0).AccountID = CLongSafe(dr.Item(7), 0).ToString
                            Dim personID As Integer = CIntSafe(dr.Item(8), 0)
                            If payment.GrossTotal < 0 Then
                                payment.Lines(0).DebitCreditIndicator = AuditFileSourceDocumentsPaymentsPaymentLineDebitCreditIndicator.K
                                If personID > 0 Then payment.Lines(0).SupplierID = personID.ToString
                                payment.GrossTotal = -payment.GrossTotal
                                creditSum = CRound(creditSum + Convert.ToDouble(payment.GrossTotal), 2)
                            Else
                                payment.Lines(0).DebitCreditIndicator = AuditFileSourceDocumentsPaymentsPaymentLineDebitCreditIndicator.D
                                If personID > 0 Then payment.Lines(0).CustomerID = personID.ToString
                                debitSum = CRound(debitSum + Convert.ToDouble(payment.GrossTotal), 2)
                            End If
                            payment.Lines(0).Description = payment.Description
                            payment.Lines(0).LineNumber = id.ToString & "-1"
                ...
                Using detailsData As DataTable = myComm.Fetch
                    ...
                    For Each dr As DataRow In myData.Rows
                        ...
                        Else
                            ...
                            document.Lines = New AuditFileSourceDocumentsMovementOfGoodsStockMovementLine() _
                                {New AuditFileSourceDocumentsMovementOfGoodsStockMovementLine}
                            document.Lines(0).TransactionID = document.SystemID
                            document.Lines(0).MovementSubType = ConvertLocalizedName(simpleType)
                            document.Lines(0).LineNumber = document.SystemID & "-1"
                            document.Lines(0).ProductCode = "PR" & CIntSafe(dr.Item(7), 0).ToString
                            document.Lines(0).AccountID = CLongSafe(dr.Item(8), 0).ToString
                            document.Lines(0).Quantity = Convert.ToDecimal(Math.Abs(CDblSafe(dr.Item(9), 2, 0.0)))
                            document.Lines(0).BookValue = Convert.ToDecimal(Math.Abs(CDblSafe(dr.Item(10), 2, 0.0)))
                            document.Lines(0).MovementComments = CStrSafe(dr.Item(11))

                            Dim personID As Integer = CIntSafe(dr.Item(12), 0)
                            If simpleType = Goods.GoodsOperationType.Acquisition Then
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.PIR
                                If personID > 0 Then document.Lines(0).SupplierID = personID.ToString
                                quantityReceived = CRound(quantityReceived + Convert.ToDouble(document.Lines(0).Quantity.Value))
                            ElseIf simpleType = Goods.GoodsOperationType.Discard Then
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.N
                                quantityIssued = CRound(quantityIssued + Convert.ToDouble(document.Lines(0).Quantity.Value))
                            ElseIf simpleType = Goods.GoodsOperationType.RedeemToSupplier Then
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.PRG
                                If personID > 0 Then document.Lines(0).SupplierID = personID.ToString
                                quantityIssued = CRound(quantityIssued + Convert.ToDouble(document.Lines(0).Quantity.Value))
                            ElseIf simpleType = Goods.GoodsOperationType.RedeemFromBuyer Then
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.PG
                                If personID > 0 Then document.Lines(0).CustomerID = personID.ToString
                                quantityReceived = CRound(quantityReceived + Convert.ToDouble(document.Lines(0).Quantity.Value))
                            ElseIf simpleType = Goods.GoodsOperationType.Transfer Then
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.PARD
                                If personID > 0 Then document.Lines(0).CustomerID = personID.ToString
                                quantityIssued = CRound(quantityIssued + Convert.ToDouble(document.Lines(0).Quantity.Value))
                            Else
                                document.Lines(0).MovementType = AuditFileSourceDocumentsMovementOfGoodsStockMovementLineMovementType.KT
                                ...
                                End If
                            End If
                            document.Lines(0).MovementTypeSpecified = True
